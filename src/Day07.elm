module Day07 exposing (partOne, partTwo)
...
partOne _ =
    List.permutations [ 0, 1, 2, 3, 4 ]
        |> List.maximumBy (\a -> performWith 0 a)
        |> Maybe.map (performWith 0)
        |> Answer.fromMaybeInt
...
partTwo _ =
    List.permutations [ 5, 6, 7, 8, 9 ]
        |> List.maximumBy (\a -> performWithLoop 0 a)
        |> Maybe.map (performWithLoop 0)
        |> Answer.fromMaybeInt
...
{-| takes in a list of settings and performs them in order, returning the output
-}
performWith : Int -> List Int -> Int
performWith input phaseSettings =
    case phaseSettings of
        [] ->
            -- If there are no phases left, return the input as output
            input
        ...
        setting :: remainingSettings ->
            let
                finalState =
                    prog
                        |> Intcode.setInputs [ setting, input ]
                        |> Intcode.executeProgram
            in
            performWith (getOutput finalState) remainingSettings
...
getOutput { outputs } =
    case outputs of
        [] ->
            Debug.todo "No output"

        [ output ] ->
            output

        _ ->
            Debug.todo "Multiple outputs?"


{-| takes in a list of settings and performs them in order, returning back
to the start until a `halt` is encountered.
-}
performWithLoop : Int -> List Int -> Int
performWithLoop input phaseSettings =
    phaseSettings
        |> List.map
            (\setting ->
                prog |> Intcode.setInputs [ setting ]
            )
        |> performWithLoopHelper 0


performWithLoopHelper : Int -> List State -> Int
performWithLoopHelper input stateList =
    if stateList |> List.all (.status >> (==) Intcode.Halted) then
        input

    else if stateList |> List.any (.status >> (==) Intcode.Halted) then
        Debug.todo "Partially halted pipeline?"

    else
        let
            ( newInput, newStateList ) =
                stateList
                    |> List.map (\state -> { state | outputs = [] })
                    |> List.mapAccuml
                        (\i state ->
                            let
                                executedState =
                                    state
                                        ...
                                        |> Intcode.executeProgram
                            in
                            ( executedState |> getOutput
                            , executedState
                            )
                        )
                        input
        in
        performWithLoopHelper newInput newStateList
