module Day03.Input exposing (Input, input, sampleInput)
...
wireParser : Parser (List Segment)
wireParser =
    succeed identity
        |= Parser.loop [] wireParserHelper


wireParserHelper : List Segment -> Parser (Step (List Segment) (List Segment))
wireParserHelper segmentList =
    oneOf
        [ succeed (\segment -> Loop (segment :: segmentList))
            |= segmentParser
        , succeed (\segment -> Loop (segment :: segmentList))
            |. token ","
            |= segmentParser
        , succeed ()
            |> map (\_ -> Done (List.reverse segmentList))
        ]
...
segmentParser =
    oneOf
        [ succeed (Tuple.pair Right) |. Parser.token "R" |= Parser.int
        , succeed (Tuple.pair Down) |. Parser.token "D" |= Parser.int
        , succeed (Tuple.pair Left) |. Parser.token "L" |= Parser.int
        , succeed (Tuple.pair Up) |. Parser.token "U" |= Parser.int
        ]


parseInput : String -> Input
parseInput raw =
    case run inputParser raw of
        Ok val ->
            val

        Err _ ->
            Debug.todo "Couldn't parse input"
