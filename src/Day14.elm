...
import Day14.Input exposing (input)
...
init name amt =
    { need = Dict.empty |> Dict.insert name amt
    , oreRequired = 0
    }


oreRequired : State -> Int
oreRequired state =
    let
        nextInput =
            state.need
                |> Dict.toList
                -- must need at least 1
                |> List.filter (\( _, i ) -> i > 0)
                |> List.head
    in
    case nextInput of
        ...
        Just ( "ORE", amount ) ->
            oreRequired
                { state
                    | oreRequired = state.oreRequired + amount
                    , need = state.need |> Dict.remove "ORE"
                }

        Just ( name, amount ) ->
            oreRequired (buy amount name state)


buy : Int -> String -> State -> State
buy amountNeeded string state =
    let
        ( inputs, output ) =
            findCost string

        multiplier =
            (amountNeeded |> toFloat)
                / (output |> Tuple.first |> toFloat)
                |> ceiling
    in
    state
        |> addNeeds
            (inputs
                |> List.map (Tuple.mapFirst <| (*) multiplier)
            )
        |> addHas
            (output
                |> (Tuple.mapFirst <| (*) multiplier)
            )


addNeeds : List ( Int, String ) -> State -> State
addNeeds stringIntList state =
    let
        mergeNeeds =
            Dict.merge
    ...
    { state
        | need =
            mergeNeeds state.need newNeeds Dict.empty
    }


addHas : ( Int, String ) -> State -> State
addHas ( amount, string ) state =
    { state
        | need =
            state.need
                |> Dict.update string
                    (\maybeAmount ->
                        case maybeAmount of
                            ...
                            Just oldAmount ->
                                Just (oldAmount - amount)
                    )
    }


sumNeeds : ( ( Int, String ), List ( Int, String ) ) -> ( Int, String )
sumNeeds ( first, rest ) =
    ( 1, "test" )


findCost : String -> ( List ( Int, String ), ( Int, String ) )
findCost string =
    case input |> List.filter (\( _, ( _, outputName ) ) -> outputName == string) of
        [ costEntry ] ->
            costEntry

        [] ->
            Debug.todo ("Cannot find " ++ string)

        _ ->
            Debug.todo ("Multiple ways to make " ++ string)
...
partTwoHelper : Int -> Int
partTwoHelper amount =
    if (oreRequired <| init "FUEL" amount) <= 1000000000000 then
        -- Keep looking
        partTwoHelper (amount * 2)

    else
        partTwoBinarySearch (amount // 2) amount


{-| min is inclusive, max is exclusive of the amount we're searching for.
-}
partTwoBinarySearch : Int -> Int -> Int
partTwoBinarySearch min max =
    let
        mid =
            (max + min) // 2
    in
    ...
    else if (oreRequired <| init "FUEL" mid) > 1000000000000 then
        partTwoBinarySearch min mid

    else
        partTwoBinarySearch mid max
