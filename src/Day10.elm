...
import Day10.Input exposing (Tile(..), input)
...
viewFrom : Coordinate -> List Coordinate -> Dict Float (List Coordinate)
viewFrom coordinate coordinateList =
    coordinateList |> Dict.groupBy (\c -> Coordinate.heading coordinate c)


bestAsteroid : Maybe ( Coordinate, Int )
bestAsteroid =
    let
        allAsteroidCoords =
            input
                |> Matrix.locations ((==) Asteroid)
    in
    allAsteroidCoords
        |> List.map
            (\coord ->
                let
                    otherAsteroids =
                        allAsteroidCoords |> List.remove coord
                in
                ( coord
                , viewFrom coord otherAsteroids |> Dict.size
                )
            )
...
partTwo _ =
    let
        ...
        asteroidView : Dict Float (List Coordinate)
        asteroidView =
            let
                otherAsteroids =
                    input
                        ...
                        |> List.remove station
            in
            viewFrom station otherAsteroids
    in
    asteroidView
        |> partTwoHelper station 200 -0.1
        |> (\( x, y ) -> x * 100 + y)
        |> String.fromInt
        |> Solved


partTwoHelper : Coordinate -> Int -> Float -> Dict Float (List Coordinate) -> Coordinate
partTwoHelper position count previousDirection asteroids =
    let
        maybeNextDirection =
            asteroids
                |> Dict.keys
                |> List.filter (\direction -> direction > previousDirection)
                |> List.sort
                |> List.head
    in
    case maybeNextDirection of
        Nothing ->
            partTwoHelper position count -0.1 asteroids

        Just dir ->
            case fireCannon position dir asteroids of
                ( Nothing, _ ) ->
                    -- Shot missed
                    partTwoHelper position count dir asteroids

                ( Just coord, newAsteroids ) ->
                    if count <= 1 then
                        coord

                    else
                        partTwoHelper position (count - 1) dir newAsteroids


{-| Fire the canon in the specified direction, returns the coordinate of
any asteroid that was destroyed as well as the new map of visible
asteroids.
-}
fireCannon : Coordinate -> Float -> Dict Float (List Coordinate) -> ( Maybe Coordinate, Dict Float (List Coordinate) )
fireCannon position direction asteroids =
    let
        aimedAt =
            Dict.get direction asteroids
    in
    case aimedAt of
        Nothing ->
            ( Nothing, asteroids )
        ...
        Just coordinateList ->
            case coordinateList |> List.sortBy (Coordinate.distance position) of
                [] ->
                    ( Nothing, asteroids |> Dict.remove direction )

                closest :: [] ->
                    ( Just closest, asteroids |> Dict.remove direction )

                closest :: remainder ->
                    ( Just closest, asteroids |> Dict.insert direction remainder )
