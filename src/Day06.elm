...
import Day06.Input exposing (Orbit, Planet, input)
...
orbitalDistance : Planet -> Planet -> List Orbit -> Int
orbitalDistance =
    Debug.todo "broken"
...
partOne _ =
    orbitCounter input [ ( "COM", 0 ) ] 0
        |> String.fromInt
        |> Solved


orbitCounter : List Orbit -> List ( Planet, Int ) -> Int -> Int
orbitCounter orbits planetsToCheck orbitCount =
    case planetsToCheck of
        [] ->
            orbitCount
        ...
        ( planet, depth ) :: tail ->
            orbitCounter
                orbits
                (List.append tail (findImmediateOrbiters orbits planet |> List.map (\p -> Tuple.pair p (depth + 1))))
                (orbitCount + depth)


{-| find all the objects directly in orbit around this planet
-}
findImmediateOrbiters : List Orbit -> Planet -> List Planet
findImmediateOrbiters orbitList planet =
    orbitList
        |> List.filter (Tuple.first >> (==) planet)
        |> List.map Tuple.second


{-| find the closest orbit "parent" to this planet
-}
immediateOrbit : List Orbit -> Planet -> Maybe Orbit
immediateOrbit orbitList planet =
    let
        parentPlanets =
            orbitList |> List.filter (Tuple.second >> (==) planet)
    in
    case parentPlanets of
        [] ->
            Nothing
        ...
        _ ->
            Debug.todo "multiple immediate orbits?"


{-| given a particular planet, return the list of planets it orbits starting with the rootPlanet
-}
orbitalList : Planet -> List Planet
orbitalList planet =
    case immediateOrbit input planet of
        Nothing ->
            []

        Just ( inner, _ ) ->
            inner :: orbitalList inner
...
partTwo _ =
    let
        youOrbits =
            "YOU" |> orbitalList |> Set.fromList

        sanOrbits =
            "SAN" |> orbitalList |> Set.fromList
    in
    Set.union (Set.diff youOrbits sanOrbits) (Set.diff sanOrbits youOrbits)
        |> Set.size
        |> String.fromInt
        |> Solved
