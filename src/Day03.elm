...
import Day03.Input exposing (..)
import Day03.Model exposing (..)
...
findIntersections ( wire1, wire2 ) =
    placeWire wire1 ( 0, 0 ) 0 Dict.empty
...
placeWire listSegment coord distance dict =
    case listSegment of
        ...
        ( _, 0 ) :: tail ->
            placeWire tail coord distance dict
...
searchWire listSegment coord distance dict =
    case listSegment of
        ...
        ( _, 0 ) :: tail ->
            searchWire tail coord distance dict
        ...
        ( dir, len ) :: tail ->
            let
                newCoord =
                    move dir coord
            in
            case Dict.get newCoord dict of
                Nothing ->
                    searchWire
                        (( dir, len - 1 ) :: tail)
                        newCoord
                ...
                Just d1 ->
                    ( newCoord, (distance + 1) + d1 )
                        :: searchWire (( dir, len - 1 ) :: tail)
                            newCoord
                            (distance + 1)
                            dict
