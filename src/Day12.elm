...
partOne _ =
    input
        |> simulate 1000
        |> List.foldl (\moon -> totalEnergy moon |> (+)) 0
        |> Answer.fromInt
...
simulate int moonList =
    if int <= 0 then
        moonList

    else
        simulate (int - 1)
            (moonList |> updateVelocities |> updatePositions)


updateVelocities : List Moon -> List Moon
updateVelocities moonList =
    moonList
        |> List.map (updateVelocity moonList)
...
updatePositions : List Moon -> List Moon
updatePositions moonList =
    moonList
